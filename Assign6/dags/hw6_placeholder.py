...
def extract(city, longitude, latitude):
    ...
    file_path = f"{clean_city}_{date_to_fetch}.csv"
...
def load(file_path, database, schema, target_table):
    ...
    try:
        ...
        cur.execute(f"DELETE FROM {target_table} WHERE date >= '{date_to_fetch}' AND date < '{next_day_of_date_to_fetch}'")
        # TO DO:
...
with DAG(
    ...
    CITY = "Los Altos"               # TO DO: Set the city name to your coordinates' city name

    database = "USER_DB_FOX"           # TO DO: Set the database name
    schema = "raw"
    target_table = "weather_data_incremental" # TO DO: Set the target table name
